...
output_csv_file = r"D:\DU Training\Case Study-Zomato\dataset-pipeline\coordinates_random.csv"

inter_output_csv_file = r"D:\DU Training\Case Study-Zomato\dataset-pipeline\coordinates_intermee.csv"
...
if os.path.isfile(csv_file_path):
    ...
    # Get a random sample of 1000 records from df and move them to df_first
    df_first = df.sample(n=100, random_state=42)
    
    # Remove the selected records from df
    df = df.drop(df_first.index)
    ...
    if __name__ == '__main__':
        ...
        df_concatenated = pd.concat([df, df_first.reset_index(drop=True)], ignore_index=True)
